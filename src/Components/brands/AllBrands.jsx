...
const AllBrands = () => {
  ...
  const { data, isLoading, error } = useGetAllBrandQuery(searchValue);
  ...
  const handleDeleteProduct = async (service) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the brand "${service?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteBrand(service?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The brand "${service?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href="/brands/add-brand">
              <button className="btn w-56">Create brands</button>
            </Link>
          </div>
        </div>
