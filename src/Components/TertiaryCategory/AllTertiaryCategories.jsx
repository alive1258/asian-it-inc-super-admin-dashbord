...
import {
  useDeleteTertiaryCategoryMutation,
  useGetAllTertiaryCategoriesQuery,
} from "@/redux/api/tertiaryCategoriesApi";

const AllTertiaryCategories = () => {
  const query = {};
  const [searchQuery, setSearchQuery] = useState(""); // State to track search input value
  const { data, error, isLoading } = useGetAllTertiaryCategoriesQuery(query);
  const [deleteTertiaryCategory] = useDeleteTertiaryCategoryMutation();
  ...
  const handleDeleteTertiaryCategory = async (tertiary_category) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the tertiary category "${tertiary_category?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteTertiaryCategory(
          tertiary_category?.id
        ).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The tertiary category "${tertiary_category?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <div className="md:px-6 px-4 pt-5 mt-6 bg-primary-base mx-6 rounded-lg">
      ...
      <div className="md:flex justify-between items-center">
        <h1 className="text-2xl font-semibold">All Tertiary Categories</h1>
        <div className="md:flex items-center space-x-4">
          ...
          <Link href="/tertiary-categories/add-tertiary-category">
            <button className="btn w-52">Create Tertiary Category</button>
          </Link>
        </div>
      </div>
          ...
          <table className="min-w-full bg-primary-base text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
            <thead>
              <tr className="bg-primary-base text-start text-[16px] ">
                ...
                <th className="py-4 px-4 text-start">Tertiary Category Name</th>
                <th className="py-4 px-4 text-start">Company</th>
                  ...
                  >
                    ...
                    <td className="my-2 px-4 text-end">
                      ...
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/sub-categories/edit-sub-category/${item?.id}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button
                          onClick={() => handleDeleteTertiaryCategory(item)}
                        >
...
export default AllTertiaryCategories;
