...
import { useDeleteCapabilitiesMutation, useGetAllCapabilitiesQuery } from "@/redux/api/capabilitiesApi";
...
const AllCapabilities = () => {
  ...
  const { data, isLoading, error } = useGetAllCapabilitiesQuery(searchValue);
  const [deleteCapabilities] = useDeleteCapabilitiesMutation();
  ...
  const handleDeleteProduct = async (service) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the capability "${service?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteCapabilities(service?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The capability "${service?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <>
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-semibold">All Capabilities</h1>
          <div className="flex items-center space-x-4">
            ...
            <Link href="/capabilities/add-capability">
              <button className="btn w-56">Create Capabilities</button>
            </Link>
          </div>
        </div>
                  ...
                  data?.data?.data?.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 ">
                        <div>
                          <p>{truncateCharacters(item?.name, 50)}</p>
                        </div>
                      </td>
                      <td className="py-3 px-4 ">
                        <div>
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/capabilities/edit/${item?.id}`}>
