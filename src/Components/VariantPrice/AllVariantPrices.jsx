...
import {
  ...
  useGetAllVariantPricesQuery,
} from "@/redux/api/variantPricesApi";

const AllVariantPrices = () => {
  const query = {};
  const [searchQuery, setSearchQuery] = useState("");
  const { data, error, isLoading } = useGetAllVariantPricesQuery(query);
  const [deleteVariantPrice] = useDeleteVariantPriceMutation();
  ...
  const handleDeleteVariant = async (variantPrice) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the variant Price "${variantPrice?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteVariantPrice(variantPrice?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The variant Price "${variantPrice?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <div className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold">All Variant Price</h1>
        <div className="flex items-center space-x-4">
          ...
          <Link href="/variants/add-variant">
            <button className="btn w-40">Create variant Price</button>
          </Link>
        </div>
      </div>
...
export default AllVariantPrices;
