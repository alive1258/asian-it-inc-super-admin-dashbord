...
const AllIndustries = () => {
  ...
  const { data, isLoading, error } = useGetAllIndustriesQuery(searchValue);
  const [deleteIndustries] = useDeleteIndustriesMutation();

  const handleSearchChange = (e) => {
    setSearchValue({ ...searchValue, search: e.target.value.toLowerCase() });
  };

  const handleDeleteProduct = async (industry) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the industry "${industry?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteIndustries(industry?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The our industry "${industry?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href="/industries/add-industry">
              <button className="btn w-56">Create industry</button>
            </Link>
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/industries/edit/${item?.id}`}>
