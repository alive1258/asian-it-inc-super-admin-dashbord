...
const AllSuppliers = () => {
  ...
  const { data, error, isLoading } = useGetAllSuppliersQuery(query);
  ...
  const handleDeleteSupplier = async (supplier) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the supplier "${supplier?.business_name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteSupplier(supplier?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The suppliers "${supplier?.business_name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <div className="md:flex justify-between items-center">
        ...
        <div className="md:flex items-center space-x-4">
          ...
          <Link href="/categories/add-category">
            <button className="btn w-44">Create Supplier</button>
          </Link>
        </div>
      </div>
          ...
          <table className="min-w-full bg-primary-base text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
            <thead>
              <tr className="bg-primary-base text-start text-[16px] ">
                ...
                <th className="py-4 px-4 text-start">Business Name</th>
                <th className="py-4 px-4 text-start">Business Owner Name</th>
                <th className="py-4 px-4 text-start">Business Owner Mobile</th>
                <th className="py-4 px-4 text-start">Business Address</th>
                <th className="py-4 px-4 text-start">Contact Person Name</th>
                <th className="py-4 px-4 text-start">Contact Person mobile</th>
              ...
              {filteredData?.length > 0 ? (
                filteredData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-5 px-4">
                      <p>{truncateCharacters(item?.business_name, 30)}</p>
                    </td>
                    <td className="py-5 px-4">
                      <p>{truncateCharacters(item?.business_owner_name, 30)}</p>
                    </td>
                    <td className="py-5 px-4">
                      <p>
                        {truncateCharacters(item?.business_owner_mobile, 30)}
                      </p>
                    </td>
                    <td className="py-5 px-4">
                      <p>{truncateCharacters(item?.business_address, 30)}</p>
                    </td>
                    <td className="py-5 px-4">
                      <p>{truncateCharacters(item?.contact_person_name, 30)}</p>
                    </td>
                    <td className="py-5 px-4">
                      <p>
                        {truncateCharacters(item?.contact_person_mobile, 30)}
                      </p>
                    </td>
                    ...
                    <td className="my-2 px-4 text-end">
                      ...
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link href={`/suppliers/edit-suppliers/${item?.id}`}>
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button onClick={() => handleDeleteSupplier(item)}>
