...
import { useDeleteTeamMutation, useGetAllTeamQuery } from "@/redux/api/teamsApi";
...
const AllTeams = () => {
  ...
  const { data, isLoading, error } = useGetAllTeamQuery(searchValue);
  ...
  const handleDeleteProduct = async (service) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the team "${service?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteTeam(service?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The team "${service?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href="/teams/add-team">
              <button className="btn w-56">Create team</button>
            </Link>
          </div>
        </div>
