...
const AllBlogs = () => {
  ...
  const { data, isLoading, error } = useGetAllBlogsQuery(searchValue);
  ...
  const handleDeleteProduct = async (faqCategory) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the blog "${faqCategory?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteBlogs(faqCategory?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The  blog "${faqCategory?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href="/blogs/blogs/add-blog">
              <button className="btn w-56">Create Blog</button>
            </Link>
          </div>
        </div>
                ...
                {data?.data?.data?.length > 0 ? (
                  data?.data?.data?.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 ">
                        <div>
                          <p>{truncateCharacters(item?.sort_description, 30)}</p>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/blogs/blogs/edit/${item?.id}`}>
