...
import { useDeleteBlogCategoryMutation, useGetAllBlogCategoryQuery } from "@/redux/api/blogCategoryApi";
...
const AllBlogCategory = () => {
  ...
  const { data, isLoading, error } = useGetAllBlogCategoryQuery(searchValue);
  ...
  const handleDeleteProduct = async (blogCategory) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the blog category "${blogCategory?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteBlogCategory(blogCategory?.id).unwrap();
        console.log(response)
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The blog category "${blogCategory?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <>
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-semibold">Blog Category</h1>
          <div className="flex items-center space-x-4">
            ...
            <Link href="/blogs/blogs-category/add-blog-category">
              <button className="btn w-56">Create blog Category</button>
            </Link>
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/blogs/blogs-category/edit/${item?.id}`}>
