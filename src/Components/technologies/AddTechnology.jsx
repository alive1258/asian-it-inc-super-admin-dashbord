...
const AddTechnology = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await createTechnology(formData).unwrap();
      if (res?.success) {
        reset();
        router.back();
        showSuccessAlert("success", "Create Technology successfully!");
      } else {
  ...
  return (
    <>
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        <h1 className="font-semibold text-[22px] text-white">Add Technology</h1>
