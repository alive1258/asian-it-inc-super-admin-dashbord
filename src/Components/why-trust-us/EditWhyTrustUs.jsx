...
const EditWhyTrustUs = ({ id }) => {
  ...
  const [updateWhTrustUs, { isLoading }] = useUpdateWhTrustUsMutation();
  const { data } = useGetSingleWhTrustUsQuery(id);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      data.photo = photo;
      const res = await updateWhTrustUs({ id, data }).unwrap();

      if (res?.success) {
        reset();
        router.back();
        setPhoto("");
        showSuccessAlert("success", "update why trust us successfully!");
      } else {
