...
import { useDeleteTagMutation, useGetAllTagsQuery } from "@/redux/api/tagsApi";
...
const AllTags = () => {
  ...
  const { data, error, isLoading } = useGetAllTagsQuery(query);
  ...
  const handleDeleteTag = async (tag) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await deleteTag(tag?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The tag "${tag?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/tags/add-tag">
            <button className="btn w-40">Create Tag</button>
          </Link>
        </div>
      </div>
          ...
          <table className="min-w-full bg-primary-base text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
            <thead>
              <tr className="bg-primary-base text-start text-[13px] ">
                ...
                <th className="py-4 px-4 text-start">Slug</th>
              ...
              {filteredData?.length > 0 ? (
                filteredData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 ">
                      <div className="w-36">
                        <p>{truncateCharacters(item?.slug, 30)}</p>
                      </div>
                    </td>
                    <td className="py-3 px-4 ">
                      <div className="w-36">
                    ...
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link href={`/tags/edit-tag/${item?.id}`}>
