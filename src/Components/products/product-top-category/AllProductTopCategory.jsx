...
} from "@/redux/api/productTopCategoryApi";

const AllProductTopCategory = () => {
  ...
  const { data, isLoading, error } =
    useGetAllProductTopCategoryQuery(searchValue);
  const [productTopCategory] = useDeleteProductTopCategoryMutation();

  const handleSearchChange = (e) => {
    setSearchValue({ ...searchValue, search: e.target.value.toLowerCase() });
  };

  const handleDeleteProduct = async (topCategory) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the product top category "${topCategory?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await productTopCategory(topCategory?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The  Product top category "${topCategory?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href="/products/product-top-category/add-product-top-category">
              <button className="btn w-56">Create top category</button>
            </Link>
          </div>
        </div>
...
export default AllProductTopCategory;
