...
const EditTopCategory = ({ id }) => {
  ...
  const [updateFaqCategory, { isLoading }] =
    useUpdateProductTopCategoryMutation();
  const { data } = useGetSingleProductTopCategoryQuery(id);
  const onSubmit = async (data) => {
    try {
      const res = await updateFaqCategory({ id, data }).unwrap();
      if (res?.success) {
        reset();
        showSuccessAlert("success", "product top category update successfully!");
      } else {
    ...
    <>
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex items-center text-[16px] py-1">
          ...
          <span>Edit Product Top Category </span>
        </div>
