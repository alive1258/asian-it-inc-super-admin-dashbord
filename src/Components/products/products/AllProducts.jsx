...
const AllProducts = () => {
  ...
  const { data, isLoading, error } = useGetAllProductQuery(searchValue);
      ...
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href="/products/products/add-product">
              <button className="btn w-56">Create Product</button>
            </Link>
          </div>
        </div>
            ...
            <table className="min-w-full bg-primary-base text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
              <thead>
                <tr className="bg-primary-base text-start text-[13px] ">
                  ...
                  <th className="py-4 px-4 text-start">overview</th>
                ...
                {data?.data?.data?.length > 0 ? (
                  data?.data?.data?.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div>
                          <p>{truncateCharacters(item?.overview, 30)}</p>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        <div>
                          <p>{truncateCharacters(item?.productTopCategory?.name, 30)}</p>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/products/products/edit/${item?.id}`}>
