...
const AllFaqs = () => {
  ...
  const { data, isLoading, error } = useGetAllFaqsQuery(searchValue);
  ...
  const handleDeleteProduct = async (faqCategory) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the faqs "${faqCategory?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteFaqs(faqCategory?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The  faqs "${faqCategory?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Link href="/faqs/faqs/add-faqs">
              <button className="btn w-56">Create Faq</button>
            </Link>
          </div>
        </div>
            ...
            <table className="min-w-full bg-primary-base text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
              <thead>
                <tr className="bg-primary-base text-start text-[13px] ">
                  ...
                  <th className="py-4 px-4 text-start">Question</th>
                  <th className="py-4 px-4 text-start">Ans</th>
                  <th className="py-4 px-4 text-start">Faq category</th>
