...
} from "@/redux/api/faqsCategoryApi";
import TableSkeleton from "@/Components/Shared/Loading/TableSkeleton";
import Pagination from "@/Components/Shared/Pagination/Pagination";

const AllFaqsCategory = () => {
  const [searchValue, setSearchValue] = useState({
    limit: 10,
    page: 1,
    search: "",
  });
  const { data, isLoading, error } = useGetAllFaqCategoryQuery(searchValue);
  ...
  const handleDeleteProduct = async (faqCategory) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the faq category "${faqCategory?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteFaqCategory(faqCategory?.id).unwrap();
        console.log(response)
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The our category "${faqCategory?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <>
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-semibold">Faq Category</h1>
          <div className="flex items-center space-x-4">
            ...
            <Link href="/faqs/faqs-category/add-faq-category">
              <button className="btn w-56">Create Faq Category</button>
            </Link>
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/faqs/faqs-category/edit/${item?.id}`}>
...
export default AllFaqsCategory;
