...
const AllOthersPageBanners = () => {
  const [searchValue, setSearchValue] = useState({
    limit: 10,
    page: 1,
    search: "",
  });
  const { data, isLoading, error } = useGetAllHeroBannerQuery(searchValue);
  ...
  const handleDeleteProduct = async (service) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the others page banner "${service?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteHeroBanner(service?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The others page banner "${service?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <>
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-semibold">All Others page banner</h1>
          <div className="flex items-center space-x-4">
            ...
            <Link href="/banners/others-page-banners/add-others-page-banner">
              <button className="btn w-56">Create Others page banner</button>
            </Link>
          </div>
        </div>
            ...
            <table className="min-w-full bg-primary-base text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
              <thead>
                <tr className="bg-primary-base text-start text-[13px] ">
                  ...
                  <th className="py-4 px-4 text-start">Link</th>
                ...
                {data?.data?.data?.length > 0 ? (
                  data?.data?.data?.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 ">
                        <div>
                          <p>{truncateCharacters(item?.link, 30)}</p>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/banners/others-page-banners/edit/${item?.id}`}>
...
export default AllOthersPageBanners;
