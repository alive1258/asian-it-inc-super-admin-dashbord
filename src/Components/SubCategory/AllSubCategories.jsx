...
const AllSubCategories = () => {
  ...
  const { data, error, isLoading } = useGetAllSubCategoriesQuery(query);
  ...
  const handleDeleteSubCategory = async (sub_category) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await deleteCategory(sub_category?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The sub category "${sub_category?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <div className="md:px-6 px-4 pt-5 mt-6 bg-primary-base mx-6 rounded-lg">
      ...
      <div className="md:flex justify-between items-center">
        <h1 className="text-2xl font-semibold">All Sub Categories</h1>
        <div className="md:flex items-center space-x-4">
          ...
          <Link href="/sub-categories/add-sub-category">
            <button className="btn w-44">Create Category</button>
          </Link>
        </div>
      </div>
          ...
          <table className="min-w-full bg-primary-base text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
            <thead>
              <tr className="bg-primary-base text-start text-[16px] ">
                ...
                <th className="py-4 px-4 text-start">Sub Category Name</th>
                  ...
                  >
                    ...
                    <td className="my-2 px-4 text-end">
                      ...
                      <div className="flex items-center justify-end w-full gap-4">
                        ...
                        <button onClick={() => handleDeleteSubCategory(item)}>
