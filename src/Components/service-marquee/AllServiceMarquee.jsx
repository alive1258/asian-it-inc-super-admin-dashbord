...
const AllServiceMarquee = () => {
  const [searchValue, setSearchValue] = useState({
    limit: 10,
    page: 1,
    search: "",
  });
  const { data, isLoading, error } = useGetAllServiceMarqueeQuery(searchValue);
  const [deleteServiceMarquee] = useDeleteServiceMarqueeMutation();
  ...
  const handleDeleteProduct = async (service) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the service  marquee "${service?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteServiceMarquee(service?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The service marquee "${service?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <>
      <section className="md:px-6 px-4 py-7 mt-6 bg-primary-base mx-6 rounded-lg">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-semibold">All Service Marquee</h1>
          <div className="flex items-center space-x-4">
            ...
            <Link href="/service-marquee/add-service-marquee">
              <button className="btn w-56">Create Service marquee</button>
            </Link>
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="my-2 px-4 text-end rounded-r-xl">
                        <div className="flex items-center justify-end w-full gap-4">
                          <Link href={`/service-marquee/edit/${item?.id}`}>
...
export default AllServiceMarquee;
