...
const EditOurWork = ({ id }) => {
  ...
  const [updateOurWork, { isLoading }] = useUpdateOurWorkMutation();
  const { data } = useGetSingleOurWorkQuery(id);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      data.photo = photo;
      const res = await updateOurWork({ id, data }).unwrap();

      if (res?.success) {
        reset();
        router.back();
        setPhoto("");
        showSuccessAlert("success", "our work update successfully!");
      } else {
