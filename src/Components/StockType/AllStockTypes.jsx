...
const AllStockTypes = () => {
  ...
  const { data, error, isLoading } = useGetAllStockTypesQuery(query);
  const [deleteStockType] = useDeleteStockTypeMutation();
  ...
  const handleDeleteStockType = async (stockType) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the stockType "${stockType?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteStockType(stockType?.id).unwrap();
        if (response?.success) {
          Swal.fire({
            title: "Deleted!",
            text: `The stockType "${stockType?.name}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <div className="md:px-6 px-4 pt-5 mt-6 bg-primary-base mx-6 rounded-lg">
      ...
      <div className="md:flex justify-between items-center">
        <h1 className="text-2xl font-semibold">All Stock Type</h1>
        <div className="md:flex items-center space-x-4">
          ...
          <Link href="/stock-types/add-stock-type">
            <button className="btn w-44">Create Stock Type</button>
          </Link>
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="my-2 px-4 text-end">
                      ...
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link href={`/stock-types/edit-stock-type/${item?.id}`}>
